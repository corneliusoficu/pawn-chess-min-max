...
class MoveValidator:
    ...
    def is_move_valid(self,matrix, move_from, move_to, current_player_color):
        ...
        if self.is_move_source_or_destination_outside_the_board(
            from_col, from_row, to_col, to_row, current_player_color): return False
        ...
        if matrix[from_row][from_col] == 0:
            self.process_error_message("No chess piece to move!", current_player_color)
            return False

        # Source is the same as destination
        if from_row == to_row and from_col == to_col:
            self.process_error_message("You cannot move pawn to the same position from which you want to move!",current_player_color)
            return False

        # The pawn tries to move sideways
        if from_row == to_row:
            self.process_error_message("The pawn cannot be moved sideways.",current_player_color)
            return False
        ...
        if color_of_piece != current_player_color:
            self.process_error_message("You cannot move the other player's pawn!",current_player_color)
            return False

        if color_of_piece == ChessBoard.WHITE_PAWN:  # the piece is white

            if from_row > to_row:  # wants to move backwards with one step
                self.process_error_message("You cannot move the pawn backwards!",current_player_color)
                return False
            ...
            if from_col == to_col and from_row == to_row - 1:  # wants to move forward with one step
                if not MoveValidator.destination_is_empty(matrix,to_row, to_col):
                    self.process_error_message("The destination {row}{column} you chose for the pawn is already taken!"
                ...
                if not MoveValidator.destination_is_empty(matrix,to_row, to_col):
                    ...
                    self.process_error_message("Cannot move pawn over the pawn in front!", current_player_color)
                    return False

            elif abs(to_row - from_row) > 1 or abs(to_col - from_col) > 1: # wants to move too far away from current position
                self.process_error_message("The destination {row}{column} is two far away for a valid move!"
                                                    .format(row = move_to[0], column = move_to[1]), current_player_color)
                return False
                ...
                if MoveValidator.destination_is_empty(matrix,to_row, to_col):
                    ...
                    self.process_error_message("White cannot capture white pawn!", current_player_color)
                    return False
        ...
        if color_of_piece == ChessBoard.BLACK_PAWN:  # the piece is black
            ...
            if from_col == to_col and from_row == to_row + 1:  # wants to move towards the white's start line with one step
                ...
                self.process_error_message("The destination {row}{column} is too far away for a valid move!"
                                                    .format(row = move_to[0], column = move_to[1]), current_player_color)
                return False
                ...
                if MoveValidator.destination_is_empty(matrix, to_row, to_col):
                    ...
                    self.process_error_message("Black cannot capture black pawn!",current_player_color)
                    return False

        return True

    def is_move_source_or_destination_outside_the_board(self,from_col, from_row, to_col, to_row, current_player_color):
        # Trying to move pawn outside the board
        if from_row < 0 or from_row >= ChessBoard.CHESS_BOARD_SIZE:
            self.process_error_message("Source row, {value}, is outside of the board!"
                                                .format(value=from_row), current_player_color)
            return True
        if from_col < 0 or from_col >= ChessBoard.CHESS_BOARD_SIZE:
            self.process_error_message("Source column, {value}, is outside of the board!"
                                                .format(value=from_col), current_player_color)
            return True
        if to_row < 0 or to_row >= ChessBoard.CHESS_BOARD_SIZE:
            self.process_error_message("Destination row, {value}, is outside of the board!"
                                                .format(value=to_row), current_player_color)
            return True
        if to_col < 0 or to_col >= ChessBoard.CHESS_BOARD_SIZE:
            self.process_error_message("Destination column, {value}, is outside of the board!"
                                                .format(value=to_row), current_player_color)
            return True
        return False
