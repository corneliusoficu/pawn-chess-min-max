    ...
    def is_move_valid(self, move_from, move_to):
        ...
        try:
            ...
            if color_of_piece == 1: # the piece is white
                ...
                elif from_col == to_col and from_row == 1 and to_row == 3: # wants to move forward with one step
                    if not MoveValidator.destination_is_empty(to_row, to_col):
                        raise Exception("The destination {row}{column} you chose for the pawn is already taken!"
                                        .format(row = move_to[0],column = move_to[1]))
            ...
            if color_of_piece == 2:  # the piece is black
                ...
                elif from_col == to_col and from_row == 6 and to_row == 4:  # wants to move forward with one step
                    if not MoveValidator.destination_is_empty(to_row, to_col):
                        raise Exception("The destination {row}{column} you chose for the pawn is already taken!"
                                        .format(row=move_from[0], column=move_from[1]))
